...
use clap::Parser;
use verve_scheduler::admission::{Config, Scheduler};
use verve_scheduler::api::{ApiState, router};
use verve_scheduler::engine::{RecordingEngine, WorkflowEngine};
use verve_scheduler::store::Store;

#[derive(Debug, Parser)]
#[command(name = "verve-scheduler", version, about = "admission control")]
struct Args {
    ...
    #[arg(long, env = "VERVE_SCHEDULER_BIND", default_value = "127.0.0.1:8081")]
    bind: String,

    /// Total slots across every worker.
    #[arg(long, env = "VERVE_TOTAL_SLOTS", default_value_t = 64)]
    total_slots: u32,
...
async fn build_engine(args: &Args) -> Result<Arc<dyn WorkflowEngine>> {
    use verve_scheduler::temporal::{TemporalConfig, TemporalEngine};
...
async fn main() -> Result<()> {
    let args = Args::parse();
    let _telemetry = verve_telemetry::init(verve_telemetry::Config::from_env("verve-scheduler"))
        .context("telemetry")?;
