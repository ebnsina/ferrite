...
use verve_scheduler::capacity::LaneShares;
use verve_scheduler::engine::RecordingEngine;
use verve_scheduler::model::{Lane, NewWork, WorkState};
...
async fn queue(store: &Store, tenant: Uuid, lane: Lane, count: usize, tag: &str) {
    for i in 0..count {
        store
            .submit(&NewWork {
                tenant_id: tenant,
                kind: verve_scheduler::model::WorkKind::Fake,
                ref_id: Uuid::now_v7(),
                spec: json!({"steps": 1}),
                lane,
                priority_key: 0,
...
async fn cancelling_stops_the_workflow_and_returns_the_slot() {
    ...
    verve_scheduler::engine::WorkflowEngine::cancel(engine.as_ref(), &workflow_id)
        .await
        .unwrap();
    store
