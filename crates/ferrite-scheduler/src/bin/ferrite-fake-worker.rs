//! A worker for `ferrite.fake`. Not video — the point of Stage 1 is to prove
...
use clap::Parser;
use ferrite_scheduler::temporal::TemporalEngine;
...
#[derive(Debug, Parser)]
#[command(name = "ferrite-fake-worker", about = "runs ferrite.fake work")]
struct Args {
...
async fn main() -> Result<()> {
    ...
    let connection = Connection::connect(
        ConnectionOptions::new(args.address.parse::<url::Url>().context("bad --address")?)
            .identity("ferrite-fake-worker".to_string())
            .build(),
    )
    .await
