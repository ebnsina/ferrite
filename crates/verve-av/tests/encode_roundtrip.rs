...
fn drain(enc: &mut Box<dyn VideoEncoder>, into: &mut Vec<verve_av::Packet>) {
    while let Some(p) = enc.receive_packet().expect("receive") {
        into.push(p);
    }
}

fn encode_all(spec: &EncodeSpec, cancel: Arc<dyn CancelSignal>) -> Vec<verve_av::Packet> {
    verve_av::init().expect("ffmpeg init");
...
fn cancellation_stops_a_running_encode_rather_than_the_next_chunk() {
    ...
    let err = enc
        ...
        .unwrap_err();
    assert!(
        matches!(err, verve_av::AvError::Cancelled),
...
fn the_rendition_record_names_what_actually_ran() {
    ...
    assert_eq!(p.ffmpeg_version, verve_av::ffmpeg_version());
