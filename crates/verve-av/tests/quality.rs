...
use std::sync::Arc;
use verve_av::NeverCancel;
use verve_av::encoder::Preset;
use verve_av::quality::{self, GATE_VMAF, Options};
...
impl Fixture {
    fn new(name: &str) -> Option<Self> {
        ...
        let dir = std::env::temp_dir().join(format!("verve-quality-{name}"));
...
fn a_faithful_encode_clears_the_top_rung_gate() {
    ...
    let spec = verve_av::EncodeSpec::new(
        ...
        verve_av::Resolution::new(video.width, video.height),
        video.frame_rate,
    )
    .with_preset(Preset::Medium)
    .with_rate_control(verve_av::RateControl::crf(18, 8_000_000));
