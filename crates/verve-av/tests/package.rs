...
use verve_av::package::{self, Input, Track};
...
fn packaged(name: &str) -> Option<(Fixture, package::Packaged, Vec<String>)> {
    ...
    let dir = std::env::temp_dir().join(format!("verve-package-{name}"));
    ...
    let info = verve_av::probe(&source).ok()?;
...
fn nothing_to_package_is_refused() {
    let err = package::run(&[], Path::new("/tmp/verve-nothing")).unwrap_err();
    assert!(matches!(err, verve_av::AvError::InvalidSpec(_)), "{err}");
}
