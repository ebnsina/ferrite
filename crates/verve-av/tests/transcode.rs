...
use verve_av::encoder::{CancelSignal, Preset};
use verve_av::transcode::{self, Output};
use verve_av::{NeverCancel, Resolution};
...
impl Fixture {
    fn new(name: &str, size: &str, seconds: u32) -> Option<Self> {
        let dir = std::env::temp_dir().join(format!("verve-transcode-{name}"));
...
fn ladder(source: &Path) -> Vec<verve_av::ladder::Step> {
    let info = verve_av::probe(source).expect("probe");
    let video = info.primary_video().expect("video stream");
    verve_av::ladder::plan(video, &verve_av::ladder::STANDARD, Preset::Ultrafast)
}
...
fn every_rung_is_a_file_that_actually_decodes() {
    ...
    for (report, step) in reports.iter().zip(&steps) {
        let info = verve_av::probe(&report.path)
...
fn keyframes_land_at_identical_times_in_every_rung() {
    ...
    for (report, step) in reports.iter().zip(&steps) {
        let info = verve_av::probe(&report.path).expect("probe output");
...
fn a_mezzanine_normalizes_to_one_clean_copy() {
    ...
    let info = verve_av::probe(&fx.source).expect("probe");
    let video = info.primary_video().expect("video stream").clone();
    let (w, h) = video.display_size();

    let spec = verve_av::EncodeSpec::new(
        verve_av::VideoCodecName::H264,
        Resolution::new(w, h),
        video.frame_rate,
    )
    ...
    let clean = verve_av::probe(&out).expect("probe mezzanine");
    ...
    assert!(
        !clean
            .warnings
            .contains(&verve_av::Warning::VariableFrameRate)
    );
}
...
fn rotation_is_baked_in_the_same_way_ffmpeg_does_it() {
    ...
    for angle in [90, 180, 270] {
        ...
        let info = verve_av::probe(&tagged).expect("probe tagged");
        let video = info.primary_video().expect("video stream");
        let (w, h) = video.display_size();
...
fn a_rotated_source_is_laddered_on_its_display_size() {
    ...
    let info = verve_av::probe(&rotated).expect("probe rotated");
    let video = info.primary_video().expect("video stream");
    assert_eq!(
        video.display_size(),
        (720, 1280),
    ...
    let steps = verve_av::ladder::plan(video, &verve_av::ladder::STANDARD, Preset::Ultrafast);
    let top = &steps[0];
    assert!(
...
fn cancellation_stops_the_run_rather_than_finishing_it() {
    ...
    assert!(matches!(err, verve_av::AvError::Cancelled), "got {err}");
}
...
fn a_bad_spec_is_refused_before_the_file_is_opened() {
    let mut spec = verve_av::EncodeSpec::new(
        verve_av::VideoCodecName::H264,
        Resolution::new(641, 360),
        verve_av::Rational::new(30, 1),
    );
    spec.threads = 1;
    ...
    assert!(
        matches!(err, verve_av::AvError::InvalidSpec(_)),
        "got {err}"
    );
}
