//! `ferrite`. Local pipeline commands need no cluster — no Temporal, no
...
#[derive(Debug, Parser)]
#[command(name = "ferrite", version, about = "ferrite — video pipeline")]
struct Cli {
...
fn main() -> ExitCode {
    ...
    // The CLI runs on laptops with none of this configured, which is fine —
    // unlike a service, it has nowhere to send telemetry and nobody reading it.
    let _guard = ferrite_telemetry::Config::from_env("ferrite-cli")
        .ok()
        .and_then(|c| ferrite_telemetry::init(c).ok());
