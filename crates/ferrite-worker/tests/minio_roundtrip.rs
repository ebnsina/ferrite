//! Stage 0's "Rust can write to MinIO". Skips unless FERRITE_S3_BUCKET is set,
//! so `cargo test` stays green on a machine with nothing running.

use ferrite_worker::storage::{self, Backend};

fn backend() -> Option<Backend> {
    std::env::var("FERRITE_S3_BUCKET").ok()?;
    Backend::from_env().ok()
}
...
async fn a_worker_can_write_read_list_and_delete() {
    let Some(backend) = backend() else {
        eprintln!("skipped: set FERRITE_S3_BUCKET to run");
        return;
    };
